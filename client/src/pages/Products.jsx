...
const CATEGORIES = ['All', 'dresses', 'shirts', 'trousers', 'kurtas', 'jackets', 'tops', 'accessories'];
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        ...
        <div className="flex items-center gap-3 self-end md:self-auto">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl border text-sm font-medium transition-all ${hasFilters ? 'border-primary text-primary bg-primary/5' : 'border-border text-foreground hover:border-primary'}`}
          >
      ...
      {showFilters && (
        ...
        >
            ...
            <div className="flex flex-wrap gap-2">
              {SIZES.map(s => (
                <button key={s} onClick={() => setFilters(f => ({ ...f, size: f.size === s ? '' : s }))}
                  className={`px-3 py-1 text-xs rounded-full border transition-all ${filters.size === s ? 'border-primary bg-primary text-white' : 'border-border hover:border-primary text-foreground'}`}>
                  {s}
                </button>
              ))}
            </div>
          ...
          <div className="flex items-end">
            {hasFilters && (
              <button onClick={clearFilters} className="flex items-center gap-1 text-sm text-muted-foreground hover:text-destructive transition-colors">
              ...
              <div className="flex justify-between items-start">
                <div className="flex-1 min-w-0 mr-2">
                  <Link to={`/products/${product._id}`}>
                    <h3 className="text-sm font-serif font-medium text-foreground hover:text-primary transition-colors truncate">{product.name}</h3>
                  </Link>
                ...
                <p className="text-sm font-bold text-primary-dark flex-shrink-0">₹{product.price?.toFixed(2)}</p>
              </div>
            </motion.div>
          ))}
        </div>
